...
export default function ProductMaster({ inventoryLots, uatMode }: { inventoryLots: InventorySourceLot[]; uatMode: boolean }) {
  ...
  const preparedComponents = useMemo(() => state.products.flatMap((product) => {
    if (product.id === selectedProductId || product.productType !== "prepared_component") return [];
