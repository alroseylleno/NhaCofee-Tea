...
import { createActiveSession, importInventoryBatch, loadInventory, migrateLocalLifecycle, removeActiveSession, removeInventory, saveInventory, settleInventoryPeriod, settleInventoryPeriodWithCarry, updateActiveSession } from "@/lib/inventory-store";
...
type Ingredient = {
  ...
  stockState?: "sealed" | "opened"; returnedOn?: string; sourceSessionId?: string; firstOpenedAt?: string;
};
...
function formatMonth(monthKey: string) { const [year, month] = monthKey.split("-"); return year && month ? `Tháng ${month}/${year}` : monthKey; }
...
function nextMonth(value: string) { const [year, month] = value.split("-").map(Number); if (!year || !month) return ""; const date = new Date(Date.UTC(year, month - 1, 1)); date.setUTCMonth(date.getUTCMonth() + 1); return date.toISOString().slice(0, 7); }
...
function VietnameseDateInput({ value, onChange, required = false, disabled = false }: { value: string; onChange: (value: string) => void; required?: boolean; disabled?: boolean }) {
  ...
  return <input disabled={disabled} required={required} type="text" inputMode="numeric" value={display} onChange={(event) => setDisplay(event.target.value)} onBlur={commit} placeholder="dd/mm/yyyy" aria-label="Ngày theo định dạng dd/mm/yyyy" />;
}
...
function seedInventoryUat(): { items: Ingredient[]; lotMeta: Record<string, LotMeta>; activeSessions: ActiveSession[] } {
  ...
  const items: Ingredient[] = [
    ...
    { id: "uat-lot-milk-previous", name: "Sữa tươi không đường", category: "Sữa", brand: "Dalat Milk", unit: "hộp", quantity: 2, specification: "1 l", unitCost: 33_000, purchasedOn: addDays(today, -35), supplier: "WinMart", receiptCode: "UAT-MILK-PREV", receipt: { name: "hoa-don-sua-tuoi-thang-truoc-uat.jpg", dataUrl: receiptPreview }, history: created("uat-lot-milk-previous", addDays(today, -35)) },
    ...
    { id: "uat-lot-matcha", name: "Matcha", category: "MATCHA", brand: "Uji", unit: "hộp", quantity: 1, specification: "800 g", conversion: { amount: 800, unit: "g" }, unitCost: 320_000, purchasedOn: addDays(today, -3), supplier: "Nhà cung cấp UAT", receiptCode: "UAT-MATCHA-800", history: created("uat-lot-matcha", addDays(today, -3)) },
  ];
  return {
    items,
    lotMeta: {
      "uat-lot-milk": { expiresOn: addDays(today, 5), shelfLifeHours: 72, storageLocation: "Tủ mát" },
      "uat-lot-milk-previous": { expiresOn: addDays(today, -5), shelfLifeHours: 72, storageLocation: "Tủ mát" },
      ...
      "uat-lot-matcha": { expiresOn: addDays(today, 90), shelfLifeHours: 720, storageLocation: "Kho khô" },
    },
    activeSessions: [
      ...
      { id: "uat-used-tea", sourceReceiptId: "uat-lot-tea", ingredientKey: ingredientKey(items[3]), activatedAt: atHours(-96), useBy: atHours(72), status: "used", closedAt: atHours(-60), reason: "used_up", note: "Dữ liệu mẫu UAT" },
      { id: "uat-waste-cream", sourceReceiptId: "uat-lot-cream", ingredientKey: ingredientKey(items[4]), activatedAt: atHours(-72), useBy: atHours(-24), status: "wasted", closedAt: atHours(-36), reason: "temperature", note: "Tủ mát mất điện - dữ liệu mẫu UAT" },
      { id: "uat-active-matcha", sourceReceiptId: "uat-lot-matcha", ingredientKey: ingredientKey(items[5]), activatedAt: atHours(-48), costRecognitionMonth: today.slice(0, 7), useBy: atHours(672), status: "active", reason: "first_open", openedAmount: 800, openedUnit: "g", provisionalCost: 320_000, note: "Mẫu chốt kỳ: nhập còn lại 600g" },
    ],
  };
}
...
export default function Home() {
  ...
  const [usedMonthFilter, setUsedMonthFilter] = useState("all");
  const [collapsedInventoryMonths, setCollapsedInventoryMonths] = useState<Set<string>>(() => new Set());
  const [inventoryCostMonth, setInventoryCostMonth] = useState(() => new Date().toISOString().slice(0, 7));
  ...
  const totalValue = items.filter((item) => item.stockState !== "opened").reduce((sum, item) => sum + item.quantity * item.unitCost, 0);
  const currentMonth = new Date().toISOString().slice(0, 7);
  const totalStockValue = items.reduce((sum, item) => sum + sealedInLot(item) * item.unitCost, 0);
  const inventoryCostMonthOptions = useMemo(() => [...new Set([
    currentMonth,
    ...activeSessions
      .filter((session) => session.status === "used" || session.status === "wasted")
      .map((session) => session.costRecognitionMonth || session.activatedAt.slice(0, 7)),
  ])].filter((month) => /^\d{4}-\d{2}$/.test(month)).sort((left, right) => right.localeCompare(left)), [activeSessions, currentMonth]);
  const selectedMonthInventoryValue = activeSessions
    .filter((session) => (session.status === "used" || session.status === "wasted") && (session.costRecognitionMonth || session.activatedAt.slice(0, 7)) === inventoryCostMonth)
    .reduce((sum, session) => sum + (session.recognizedCost ?? items.find((item) => item.id === session.sourceReceiptId)?.unitCost ?? 0), 0);
  ...
  const inventoryViewGroups = useMemo(() => filteredGroups.map((group) => {
    const lots = group.lots.filter((lot) => {
      ...
      if (inventoryView === "active") return activeSessions.some((session) => session.status === "active" && session.sourceReceiptId === lot.id);
      if (inventoryView === "used") return activeSessions.some((session) => session.status === "used" && session.sourceReceiptId === lot.id);
      if (inventoryView === "unconverted") return !lot.conversion?.amount && (sealedInLot(lot) > 0 || activeSessions.some((session) => session.status === "active" && session.sourceReceiptId === lot.id));
      return activeSessions.some((session) => session.status === "wasted" && session.sourceReceiptId === lot.id);
    });
    return { ...group, lots };
  }).filter((group) => group.lots.length > 0), [filteredGroups, inventoryView, activeSessions, items]);
  const inventoryMonthOptions = useMemo(() => [...new Set([
    ...items.map((item) => item.purchasedOn.slice(0, 7)),
    ...activeSessions.filter((session) => session.status === "used" && session.closedAt).map((session) => session.closedAt!.slice(0, 7)),
  ].filter(Boolean))].sort((left, right) => right.localeCompare(left)), [items, activeSessions]);
  const inventoryTimedGroups = useMemo(() => inventoryViewGroups.map((group) => ({
    ...group,
    lots: group.lots.filter((lot) => {
      const matchesPurchase = purchaseMonthFilter === "all" || lot.purchasedOn.startsWith(purchaseMonthFilter);
      const matchesUsed = usedMonthFilter === "all" || activeSessions.some((session) => session.status === "used" && session.sourceReceiptId === lot.id && session.closedAt?.startsWith(usedMonthFilter));
      return matchesPurchase && matchesUsed;
    }),
  })).filter((group) => group.lots.length > 0), [inventoryViewGroups, purchaseMonthFilter, usedMonthFilter, activeSessions]);
  const inventoryCategoryCounts = useMemo(() => inventoryTimedGroups.reduce<Record<string, number>>((counts, group) => ({ ...counts, [group.category]: (counts[group.category] || 0) + 1 }), {}), [inventoryTimedGroups]);
  const inventoryGroups = useMemo(() => inventoryTimedGroups.filter((group) => inventoryCategory === "all" || group.category === inventoryCategory).sort((left, right) => {
  ...
  }), [inventoryTimedGroups, inventoryCategory, inventorySort]);
  ...
  const inventorySections = useMemo(() => {
    const months = new Map<string, Map<string, IngredientGroup[]>>();
    for (const group of inventoryGroups) {
      const lotsByMonth = new Map<string, Ingredient[]>();
      for (const lot of group.lots) {
        const month = lot.purchasedOn.slice(0, 7);
        lotsByMonth.set(month, [...(lotsByMonth.get(month) || []), lot]);
      }
      for (const [month, lots] of lotsByMonth) {
        const categories = months.get(month) || new Map<string, IngredientGroup[]>();
        categories.set(group.category, [...(categories.get(group.category) || []), { ...group, lots }]);
        months.set(month, categories);
      }
    }
    return [...months.entries()].sort(([left], [right]) => right.localeCompare(left)).map(([month, categories]) => ({
      month,
      categories: [...categories.entries()].sort(([left], [right]) => left.localeCompare(right, "vi")).map(([category, groups]) => ({
        category,
  ...
  }, [inventoryGroups, inventorySort, activeSessions, items]);
  const selectedGroupLotMonths = useMemo(() => {
    if (!selectedGroup) return [] as Array<{ month: string; lots: Ingredient[] }>;
    const months = new Map<string, Ingredient[]>();
    for (const lot of selectedGroup.lots) {
      const month = lot.purchasedOn.slice(0, 7);
      months.set(month, [...(months.get(month) || []), lot]);
    }
    return [...months.entries()].sort(([left], [right]) => right.localeCompare(left)).map(([month, lots]) => ({ month, lots }));
  }, [selectedGroup]);
  const now = Date.now();
  ...
  async function activate(group: IngredientGroup, lot: Ingredient, reason: string, costRecognitionMonth: string, note = "") {
    ...
    const nextSession: ActiveSession = { id: crypto.randomUUID(), sourceReceiptId: lot.id, ingredientKey: group.key, activatedAt, costRecognitionMonth, useBy: useByFor(lot.firstOpenedAt || activatedAt, lotMeta[lot.id]), status: "active", reason, openedAmount: lot.conversion?.amount, openedUnit: lot.conversion?.unit, provisionalCost: lot.unitCost, note: note.trim() || undefined };
  ...
  function requestSettlement(activeSession: ActiveSession) {
    ...
    if (!lot || !openingAmount || !openingUnit) {
      window.alert("NVL này chưa có Quy đổi nên chưa thể chốt phần sử dụng thực tế. Hãy bổ sung Quy đổi trước.");
      return;
    }
  ...
  async function confirmSettlement(event: FormEvent<HTMLFormElement>) {
    ...
    if (settlementDate < settlementCandidate.activatedAt.slice(0, 10)) {
      window.alert("Ngày chốt kỳ không được trước ngày mở nguyên liệu.");
      return;
    }
    ...
    try {
      if (!isLocalUat) {
        ...
        if (settlementDisposition === "carry" && remainingAmount > 0) {
          if (settlementMode !== "month" || !returnedLotId || !continuedSessionId) throw new Error("Chuyển tiếp kỳ chỉ áp dụng khi chốt tháng và còn số lượng dư.");
          await settleInventoryPeriodWithCarry({ sessionId: settlementCandidate.id, periodEnd: settlementDate, remainingAmount, returnedLotId, continuedSessionId, historyId: crypto.randomUUID() });
        } else {
  ...
  return <main className="app-workspace">
    ...
    {workspace === "finance" && canAccessFinance ? <FinanceModule
      uatMode={isLocalUat}
      inventoryLots={items.map((item) => ({ id: item.id, name: item.name, category: item.category, quantity: item.quantity, unit: item.unit, unitCost: item.unitCost, purchasedOn: item.purchasedOn, receiptCode: item.receiptCode, internalReturn: item.stockState === "opened", availableFrom: item.returnedOn }))}
      inventorySessions={activeSessions.map((entry) => ({ id: entry.id, sourceReceiptId: entry.sourceReceiptId, activatedAt: entry.activatedAt, costRecognitionMonth: entry.costRecognitionMonth, status: entry.status, closedAt: entry.closedAt, reason: entry.reason, recognizedCost: entry.recognizedCost }))}
    ...
    {tab === "inventory" && <section className="content">
      ...
      <div className="inventory-views" role="tablist" aria-label="Lọc theo trạng thái"><button type="button" role="tab" aria-selected={inventoryView === "stock"} className={inventoryView === "stock" ? "selected" : ""} onClick={() => setInventoryView("stock")}>Kho</button><button type="button" role="tab" aria-selected={inventoryView === "active"} className={inventoryView === "active" ? "selected" : ""} onClick={() => setInventoryView("active")}>Đang dùng</button><button type="button" role="tab" aria-selected={inventoryView === "used"} className={inventoryView === "used" ? "selected" : ""} onClick={() => setInventoryView("used")}>Đã dùng</button><button type="button" role="tab" aria-selected={inventoryView === "wasted"} className={inventoryView === "wasted" ? "selected" : ""} onClick={() => setInventoryView("wasted")}>Hao hụt</button><button type="button" role="tab" aria-selected={inventoryView === "unconverted"} className={inventoryView === "unconverted" ? "selected" : ""} onClick={() => setInventoryView("unconverted")}>Chưa quy đổi</button></div>
      <div className="inventory-category-tabs" role="tablist" aria-label="Lọc theo category"><button type="button" role="tab" aria-selected={inventoryCategory === "all"} className={inventoryCategory === "all" ? "selected" : ""} onClick={() => setInventoryCategory("all")}>Tất cả ({inventoryTimedGroups.length})</button>{inventoryCategories.map((category) => <button type="button" role="tab" aria-selected={inventoryCategory === category} className={inventoryCategory === category ? "selected" : ""} onClick={() => setInventoryCategory(category)} key={category}>{category} ({inventoryCategoryCounts[category] || 0})</button>)}</div>
      ...
      <div className="inventory-month-filters"><label><span>Tháng nhập</span><select value={purchaseMonthFilter} onChange={(event) => setPurchaseMonthFilter(event.target.value)}><option value="all">Tất cả tháng</option>{inventoryMonthOptions.map((month) => <option value={month} key={`purchase-${month}`}>Tháng {month.slice(5)}/{month.slice(0, 4)}</option>)}</select></label><label><span>Tháng dùng hết</span><select value={usedMonthFilter} onChange={(event) => setUsedMonthFilter(event.target.value)}><option value="all">Tất cả tháng</option>{inventoryMonthOptions.map((month) => <option value={month} key={`used-${month}`}>Tháng {month.slice(5)}/{month.slice(0, 4)}</option>)}</select></label></div>
      {!loaded ? <p className="empty">Đang tải kho...</p> : inventorySections.length === 0 ? <div className="empty"><b>{items.length ? ({ stock: "Không còn nguyên liệu trong kho", active: "Chưa có nguyên liệu đang dùng", used: "Chưa có nguyên liệu đã dùng", wasted: "Chưa có nguyên liệu hao hụt", unconverted: "Không có nguyên liệu chưa quy đổi" } as const)[inventoryView] : "Kho đang trống"}</b><span>{items.length ? "Thử đổi tháng, trạng thái hoặc tìm bằng từ khóa khác." : "Nhấn dấu + để ghi nhận lần mua nguyên liệu đầu tiên."}</span></div> : <div className="inventory-timeline">{inventorySections.map((monthSection) => <details className="inventory-month-section" open={!collapsedInventoryMonths.has(monthSection.month)} onToggle={(event) => { const isOpen = event.currentTarget.open; setCollapsedInventoryMonths((current) => { const isCollapsed = current.has(monthSection.month); if (isOpen === !isCollapsed) return current; const next = new Set(current); if (isOpen) next.delete(monthSection.month); else next.add(monthSection.month); return next; }); }} key={monthSection.month}><summary className="inventory-month-heading"><span>{formatMonth(monthSection.month)}</span><b>{monthSection.categories.reduce((sum, category) => sum + category.groups.length, 0)} card NVL</b></summary><div className="inventory-month-content">{monthSection.categories.map((categorySection) => <section className="inventory-category-section" key={`${monthSection.month}-${categorySection.category}`}><div className="inventory-category-heading"><h3>{categorySection.category}</h3><span>{categorySection.groups.length} NVL</span></div><div className="inventory-list">{categorySection.groups.map((group) => {
        const fullGroup = ingredientGroups.find((entry) => entry.key === group.key) || group; const lotIds = new Set(group.lots.map((lot) => lot.id)); const sealed = sealedInGroup(group); const active = openSessions.filter((session) => lotIds.has(session.sourceReceiptId)).length; const used = usedSessions.filter((session) => lotIds.has(session.sourceReceiptId)).length; const wasted = wastedSessions.filter((session) => lotIds.has(session.sourceReceiptId)).length; const latestLot = [...group.lots].sort((a, b) => b.purchasedOn.localeCompare(a.purchasedOn))[0]; const readyLot = [...group.lots].filter((lot) => sealedInLot(lot) > 0).sort((left, right) => Number(right.stockState === "opened") - Number(left.stockState === "opened") || left.purchasedOn.localeCompare(right.purchasedOn))[0]; const originalQuantity = group.lots.filter((lot) => lot.stockState !== "opened").reduce((sum, lot) => sum + lot.quantity, 0); const historyMonths = new Set(fullGroup.lots.map((lot) => lot.purchasedOn.slice(0, 7))).size;
        return <details className="ingredient-summary" key={`${monthSection.month}-${group.key}`}><summary><span className="summary-name"><b>{group.name}</b><small>{group.brand} · {group.specification} · {group.lots.length} lần nhập trong tháng</small></span>{readyLot && inventoryView === "stock" && <button type="button" className="quick-activate-button" onClick={(event) => { event.preventDefault(); event.stopPropagation(); requestActivation(fullGroup, readyLot); }}>{readyLot.stockState === "opened" && readyLot.conversion ? `Mở lại ${conversionLabel(readyLot.conversion)}` : `Mở 1 ${readyLot.unit}`} <b>{sealed}/{originalQuantity}</b></button>}<span className="summary-counts"><span><b>Kho:</b> {sealed.toLocaleString("vi-VN")}</span><span className={active ? "has-active" : ""}><b>Đang dùng:</b> {active}</span><span className={used ? "has-used" : ""}><b>Đã dùng:</b> {used}</span><span className={wasted ? "has-waste" : ""}><b>Hao hụt:</b> {wasted}</span></span></summary><div className="ingredient-group-items">{[...group.lots].sort((left, right) => (right.returnedOn || right.purchasedOn).localeCompare(left.returnedOn || left.purchasedOn)).map((lot) => <div className="ingredient-lot-row" key={lot.id}><span><b>{lot.receiptCode || "Chưa có mã phiếu"}{lot.stockState === "opened" && <em className="opened-stock-badge">Đã mở</em>}</b><small>{lot.stockState === "opened" ? `Hoàn kho ${formatDate(lot.returnedOn || lot.purchasedOn)}` : `Nhập ${formatDate(lot.purchasedOn)}`} · còn {sealedInLot(lot).toLocaleString("vi-VN")} {lot.unit}{lot.stockState === "opened" && lot.conversion ? ` (${conversionLabel(lot.conversion)})` : ""}</small></span><div>{sealedInLot(lot) > 0 && <button type="button" className="lot-activate-button" onClick={() => requestActivation(fullGroup, lot)}>{lot.stockState === "opened" ? "Mở lại" : "Mở 1"}</button>}<button type="button" onClick={() => setDetailLot(lot)}>Chi tiết</button><button type="button" onClick={() => copyIngredient(lot)}>Copy</button></div></div>)}<button type="button" className="ingredient-history-button" onClick={() => setDetailGroup(fullGroup)}>Xem toàn bộ lịch sử · {fullGroup.lots.length} lần nhập / {historyMonths} tháng →</button></div></details>;
      })}</div></section>)}</div></details>)}</div>}
    </section>}
