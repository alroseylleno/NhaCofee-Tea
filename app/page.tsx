...
type FormValues = { name: string; category: string; categoryIsCustom: boolean; brand: string; brandIsCustom: boolean; invoiceCode: string; unit: string; quantity: string; specificationAmount: string; specificationUnit: string; specificationNote: string; conversionAmount: string; conversionUnit: string; unitCost: string; purchasedOn: string; supplier: string; supplierIsCustom: boolean; expiresOn: string; shelfLifeValue: string; shelfLifeUnit: ShelfLifeUnit; storageLocation: string };
...
const formDefaults = (): FormValues => { const purchasedOn = new Date().toISOString().slice(0, 10); return { name: "", category: "", categoryIsCustom: false, brand: "", brandIsCustom: false, invoiceCode: "", unit: "chai", quantity: "", specificationAmount: "", specificationUnit: "ml", specificationNote: "", conversionAmount: "", conversionUnit: "ml", unitCost: "", purchasedOn, supplier: "", supplierIsCustom: false, expiresOn: defaultExpiryFor(purchasedOn), shelfLifeValue: "", shelfLifeUnit: "days", storageLocation: "Tủ mát" }; };
...
function FieldHint({ label, help }: { label: string; help: string }) {
  return <span className="field-hint">{label}<button type="button" className="field-help" aria-label={`Hướng dẫn cho ${label}`} data-tooltip={help}>?</button></span>;
}
...
export default function Home() {
  ...
  const currentMonthInventoryValue = activeSessions
    .filter((session) => session.status !== "active" && (session.costRecognitionMonth || session.activatedAt.slice(0, 7)) === currentMonth)
  ...
  const inventoryGroups = useMemo(() => filteredGroups.filter((group) => {
    if (inventoryCategory !== "all" && group.category !== inventoryCategory) return false;
  ...
  }), [filteredGroups, inventoryCategory, inventoryView, activeSessions, items]);
  ...
  function copyIngredient(item: Ingredient) { const meta = lotMeta[item.id]; const parsedSpecification = parseSpecification(item.specification); const shelfLife = shelfLifeFormValues(meta?.shelfLifeHours); const purchasedOn = new Date().toISOString().slice(0, 10); setForm({ name: item.name, category: item.category, categoryIsCustom: false, brand: item.brand, brandIsCustom: false, invoiceCode: "", unit: item.unit, quantity: String(item.quantity), specificationAmount: parsedSpecification.amount, specificationUnit: parsedSpecification.unit, specificationNote: parsedSpecification.note, conversionAmount: item.conversion?.amount ? String(item.conversion.amount) : "", conversionUnit: item.conversion?.unit || "ml", unitCost: formatPriceInput(String(item.unitCost)), purchasedOn, supplier: item.supplier, supplierIsCustom: false, expiresOn: defaultExpiryFor(purchasedOn), shelfLifeValue: shelfLife.value, shelfLifeUnit: shelfLife.unit, storageLocation: meta?.storageLocation || "Tủ mát" }); setReceipt(undefined); setReceiptFile(undefined); setEditingId(undefined); setShowForm(true); }
  function openEdit(item: Ingredient) { const meta = lotMeta[item.id]; const parsedSpecification = parseSpecification(item.specification); const shelfLife = shelfLifeFormValues(meta?.shelfLifeHours); setForm({ name: item.name, category: item.category, categoryIsCustom: !categories.includes(item.category), brand: item.brand, brandIsCustom: !brands.includes(item.brand), invoiceCode: item.receiptCode || "", unit: item.unit, quantity: String(item.quantity), specificationAmount: parsedSpecification.amount, specificationUnit: parsedSpecification.unit, specificationNote: parsedSpecification.note, conversionAmount: item.conversion?.amount ? String(item.conversion.amount) : "", conversionUnit: item.conversion?.unit || "ml", unitCost: formatPriceInput(String(item.unitCost)), purchasedOn: item.purchasedOn, supplier: item.supplier, supplierIsCustom: !suppliers.includes(item.supplier), expiresOn: meta?.expiresOn || defaultExpiryFor(item.purchasedOn), shelfLifeValue: shelfLife.value, shelfLifeUnit: shelfLife.unit, storageLocation: meta?.storageLocation || "Tủ mát" }); setReceipt(item.receipt); setEditingId(item.id); setShowForm(true); }
  ...
  async function saveIngredient(event: FormEvent<HTMLFormElement>) {
    ...
    const next = { name: form.name.trim(), category: form.category.trim() || "Chưa phân loại", brand: form.brand.trim() || "Chưa ghi thương hiệu", receiptCode: enteredInvoiceCode || current?.receiptCode, unit: form.unit, quantity, specification: buildSpecification(form.specificationAmount, form.specificationUnit, form.specificationNote), conversion: conversionAmount ? { amount: conversionAmount, unit: form.conversionUnit } : undefined, unitCost, purchasedOn: form.purchasedOn, supplier: form.supplier.trim() || "Chưa ghi nhà cung cấp", receipt };
  ...
  async function importInventoryExcel(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const imported = rows.slice(headerIndex + 1).filter((row) => String(row[required.name]).trim()).map((row, index) => {
        ...
        const item: Ingredient = { id: crypto.randomUUID(), name: String(row[required.name]).trim(), category: String(read(row, "category")).trim() || "Chưa phân loại", brand: String(read(row, "thuong hieu")).trim() || "Chưa ghi thương hiệu", unit: String(row[required.unit]).trim(), quantity, specification, conversion: parsedConversion, unitCost, purchasedOn, supplier: String(read(row, "nha cung cap")).trim() || "Chưa ghi nhà cung cấp", receiptCode, history: [] };
  ...
  return <main>
    ...
    {tab === "inventory" && <section className="content">
      ...
      <div className="inventory-category-tabs" role="tablist" aria-label="Lọc theo category"><button type="button" role="tab" aria-selected={inventoryCategory === "all"} className={inventoryCategory === "all" ? "selected" : ""} onClick={() => setInventoryCategory("all")}>Tất cả</button>{inventoryCategories.map((category) => <button type="button" role="tab" aria-selected={inventoryCategory === category} className={inventoryCategory === category ? "selected" : ""} onClick={() => setInventoryCategory(category)} key={category}>{category}</button>)}</div>
