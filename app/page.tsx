...
function seedInventoryUat(): { items: Ingredient[]; lotMeta: Record<string, LotMeta>; activeSessions: ActiveSession[] } {
  ...
  return {
    ...
    activeSessions: [
      ...
      { id: "uat-active-matcha", sourceReceiptId: "uat-lot-matcha", ingredientKey: ingredientKey(items[4]), activatedAt: atHours(-48), costRecognitionMonth: today.slice(0, 7), useBy: atHours(672), status: "active", reason: "first_open", openedAmount: 800, openedUnit: "g", provisionalCost: 320_000, note: "Mẫu chốt kỳ: nhập còn lại 600g" },
    ],
  };
}
...
export default function Home() {
  ...
  const inventoryViewGroups = useMemo(() => filteredGroups.filter((group) => {
    if (inventoryView === "stock") return sealedInGroup(group) > 0;
    if (inventoryView === "active") return activeInGroup(group).length > 0;
    if (inventoryView === "used") return usedInGroup(group).length > 0;
    return wastedInGroup(group).length > 0;
  ...
  const inventoryTimedGroups = useMemo(() => inventoryViewGroups.filter((group) => {
    const matchesPurchase = purchaseMonthFilter === "all" || group.lots.some((lot) => lot.purchasedOn.startsWith(purchaseMonthFilter));
    const matchesUsed = usedMonthFilter === "all" || group.lots.some((lot) => activeSessions.some((session) => session.status === "used" && session.sourceReceiptId === lot.id && session.closedAt?.startsWith(usedMonthFilter)));
    return matchesPurchase && matchesUsed;
  }), [inventoryViewGroups, purchaseMonthFilter, usedMonthFilter, activeSessions]);
  ...
  return <main className="app-workspace">
    ...
    {workspace === "finance" && canAccessFinance ? <FinanceModule
      uatMode={isLocalUat}
      inventoryLots={items.map((item) => ({ id: item.id, name: item.name, quantity: item.quantity, unit: item.unit, unitCost: item.unitCost, purchasedOn: item.purchasedOn, receiptCode: item.receiptCode, internalReturn: item.stockState === "opened", availableFrom: item.returnedOn }))}
