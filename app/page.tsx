...
const formDefaults = (): FormValues => { const purchasedOn = new Date().toISOString().slice(0, 10); return { name: "", category: "", brand: "", unit: "chai", quantity: "", specificationAmount: "", specificationUnit: "ml", specificationNote: "", unitCost: "", purchasedOn, supplier: "", expiresOn: defaultExpiryFor(purchasedOn), shelfLifeValue: "", shelfLifeUnit: "days", storageLocation: "Tủ mát" }; };
...
export default function Home() {
  ...
  async function activate(group: IngredientGroup, lot: Ingredient, reason: string, note = "") {
    const activatedAt = new Date().toISOString();
    const nextSession: ActiveSession = { id: crypto.randomUUID(), sourceReceiptId: lot.id, ingredientKey: group.key, activatedAt, useBy: useByFor(activatedAt, lotMeta[lot.id]), status: "active", reason, note: note.trim() || undefined };
  ...
  function requestActivation(group: IngredientGroup, lot: Ingredient) {
    ...
    else void activate(group, lot, "first_open");
  }
  async function confirmActivation(event: FormEvent<HTMLFormElement>) {
    ...
    await activate(activationCandidate.group, activationCandidate.lot, activationReason, activationNote);
  }
