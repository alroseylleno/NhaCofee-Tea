...
- **Platform reconciliation is LIVE in Production as of 2026-09-07.** The `Nền tảng` tab, the three-file SAPO import (invoice list with items + price book), browser PDF ingest and đối soát now run in both modes; Production persists everything through Supabase (`finance_platform_order_rows.items`, `finance_grab_reconciliations`, `finance_grab_daily_reports`, `finance_counter_prices`, `replace_finance_import_bundle_v2`, `replace_finance_counter_prices`). The old `bd46caa` gates are removed. The `Quét thư mục local` button and `/api/grab-reports` route work on any local host — `http://prod.localhost:3001` runs the PRODUCTION data mode from the local dev server, so a folder scan there writes straight to Supabase (button labelled `→ ghi Production`); Vercel builds still 403 the route. Still guarded: the `grab:*`/`sapo:*` fetch scripts (a dedicated Supabase account for scripts is the one missing precondition for `grab:prod`).
