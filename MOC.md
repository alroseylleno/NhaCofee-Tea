...
| Sàn order reconciliation | `finance_grab_reconciliations` with authenticated CRUD RLS, plus `discounts` (layered discount stack), `settlement` (Grab PDF snapshot) and `counter_price` | `20260907000200` |
| SAPO platform invoice detail | `finance_platform_order_rows`, four-file atomic import RPC, import metadata and authenticated CRUD RLS; used by platform dashboards and the GRAB picker | `20260907000300` |
| Grab daily report ledger | `finance_grab_daily_reports`, one row per imported PDF with day-level marketing lines and unmatched orders | `20260907000200` |
