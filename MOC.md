...
- `Định lượng` is stored as one free-text string and split by `parseSpecification` into amount, unit and note, then rebuilt by `buildSpecification` on every save. The unit pattern must stay open (`[^\s\d]+`): a whitelist cannot cover custom units (cuộn, tờ, tem, túi), and `\b` is ASCII-only so it cut Vietnamese words open after a number ("1 lần" → unit `l`, note `ần`). Anything the pattern fails to parse falls into the note and the next save re-appends the amount, so a parser gap compounds the field on every edit rather than failing visibly.
...
- **Nền tảng dashboards were re-cut on 2026-09-14.** Removed: `Nhịp doanh thu sàn` (daily revenue bars), `Phí nền tảng / giao hàng` (take-rate ring) and `Chênh lệch SAPO ↔ Thực nhận sàn` (weekly gap stack). `Phí sàn theo nền tảng` and `Chi phí marketing Grab` now share one insight grid row. New full-width `HIỆU QUẢ QUẢNG CÁO · CPO` panel (`adsEfficiency`/`adsTotals`/`adsBreakEvenCpo`/`adsCogs` in `app/finance-module.tsx`) buckets `grabDailyReports` by day/week/month with an extra from–to date filter, and draws budget-ceiling bars behind two CPO lines plus a break-even rule, with a hover tooltip naming each series. Only `platform === "grab"` reports feed it — ShopeeFood and GreenSM settle no day-level advertising, so folding them in would dilute CPO. **CPO counts only true ad lines** — `keywordAdsSpend` keeps Marketing rows matching Keywords/ADS-xxx/Image Ads and drops co-fund service fees (Đồng tài trợ...), GrabNgonRẻ day-level lumps and Thúc Đẩy Doanh Số, which are promo costs, not advertising (legacy rows without line detail fall back to totalMarketing). Both reconciliation comparisons — list columns AND order modal — settled 2026-09-14 on ONE basis: net after the day's marketing allocation (received − allocated marketing) versus counter price / basket COGS, live from the form in the modal. Ads are a real cost of the order even though Grab bills day-level; the allocation being an even split is stated in the table disclaimer. (Same-day earlier state compared on received; superseded.) Average CPO is spend ÷ orders; **marginal CPO is (Δspend ÷ Δorders) between consecutive buckets and is deliberately computed in both directions** — scaling down reports what the lost orders were costing — but is suppressed when the ratio is negative or the order count barely moved, because those produce a precise-looking number with no meaning. Break-even CPO is `totalPayout / orders − adsCogs.perOrder`: the settlement the sàn actually transferred before advertising, minus real basket cost from `cogsFromItems` (Product Master recipes). Unlike the order-detail column, the ads panel accepts partially covered baskets — unpriced items are skipped, so the COGS is a floor and the panel prints its own coverage ratio. The from–to fields OVERRIDE the page-level period selector rather than narrowing inside it — with `adsFrom`/`adsTo` set the panel reads `state.grabDailyReports` (and unbounded `platformOrders` for COGS) instead of the period-filtered slices, so a range crossing months works and a red `Khoảng riêng` flag appears in the header saying which period is being ignored. Quick ranges (7/14/30 ngày · Toàn kỳ) anchor on the newest Grab report rather than on today, because the settlement mail lands a day late and a today-anchored window would clip the newest bar; the two date fields call `showPicker()` on click/focus since Chrome otherwise only opens the calendar from its tiny icon and Safari offers none. The daily budget ceiling has no source in any settlement file (Grab's PDF records spend, never the cap) so it stays a user-entered field defaulting to 200.000đ; hardcoding it would misreport the pre-07/09 period when the cap was 450.000đ.
...
- Product detail uses progressive saving: price, recipe rows and packaging rows can be saved independently even while other data is missing, unavailable or cannot yet produce theoretical COGS. Missing price/NVL/unit data remains a non-blocking quality warning and keeps cost/margin in `Chưa đủ`; it must not prevent saving the data already entered. The one exception is a `prepared_component` SKU's batch output: saving without an output quantity is blocked, because the picker filters such a version out and it has no cost per ml, so the SKU would vanish from `Thêm công thức nền` with no visible error.
- A `Công thức nền` SKU only appears in another product's `Thêm công thức nền` picker when its active recipe version has BOTH `output_quantity > 0` and `output_unit`. Every path that opens a recipe draft must seed `recipeOutputQuantity`/`recipeOutputUnit` — `startRecipeDraft`, `selectRecipeVersion` and the clone in `createManualProduct`. The clone omitted them until `20260912000100`, so the first save after a Copy wrote `Number("") || undefined` and silently hid 7 Syrup Mứt SKUs in Production. The SQL guard that should have caught it was a no-op: `output_quantity > 0` is NULL when the quantity is NULL and a CHECK constraint only rejects FALSE, so the null-ness is now tested explicitly and `save_product_recipe_version` raises on a half-filled pair.
- Validation warnings from `productValidationErrors` render on both the desktop table rows and the mobile product cards. A rule that hides a SKU from a picker must be visible in the catalog list, not only in the detail panel.
- Cloning a product prompts for SKU, name, Category and variant. Variant must be asked for, not inherited: product detail has no variant field, so a clone that silently kept the source variant rendered as `Cà phê đen - L · M` with no way to correct it in the UI.
- Kho NVL stores tea and coffee as BREWED volume, not dry leaf: `Trà đen số 9 HOA TRÂN` 500g converts to 12.000 ml and `Café LEELONG` 500g to 2.000 ml, and recipes draw on that directly (`Trà đào` = 150 ml of that tea). Never model a separate "ủ trà" or "cốt cà phê" prepared component on top — the brewing is already inside the conversion, and doing so prices a millilitre at ~1,1đ instead of the real 4,167đ.
- Water is a `Khác` exception line priced at 1đ, never a zero-cost ingredient: `recipeItemCost` returns undefined for any price ≤ 0 and one undefined line makes the WHOLE product's COGS read "Chưa đủ".
- Every sellable SKU carries a packaging set alongside its ingredients — Ly, nắp, ống hút, Giấy chống tràn, Túi ly, Chén topping, sized M (Ly 500ml / nắp 500 / phi 8) or L (Ly 700ml / nắp 700 / phi 12). A recipe imported from a formula workbook has none of these and must inherit them from a same-size sibling or COGS is short by 3–4k a cup.
- The ads CPO chart distinguishes two things a blank used to conflate: `không đo được` (no comparable change, or the first bucket) and `bão hoà` — spend rose while orders did not. Marginal CPO is infinite there, so it cannot be plotted, but it is the most actionable reading on the chart and is now drawn as a marker at the top of the axis with the wasted amount named (`bão hoà · +30.788 ₫, 0 đơn`).
- Kho NVL accepts a free-typed conversion unit, but Product Master only honours a unit that resolves in `unitFactors` — `ingredientDraftsFromInventory` drops anything else and the NVL then reads `CHƯA QUY ĐỔI` with no stated reason. That is how `Đào hộp BEST VALUE` sat unusable on a lot that plainly showed `40 Lát`. The lot form now warns as soon as the typed unit is unknown, and `lát`, `ly`, `tem` were added to `COUNT_UNITS` because the shop genuinely counts in them. When a unit the shop uses is missing, add it to the table — do not make the lot be rewritten to fit the table.
- There must be exactly ONE list of units. `specificationBase` kept a second, hand-copied regex whitelist that `lát`/`ly`/`tem` were never added to, so a lot carrying only a `Định lượng` (no explicit conversion) fell back to its purchase unit and the NVL read `CHƯA QUY ĐỔI` even though Kho showed a unit. The pattern is now built from `ALL_RECIPE_UNITS`, longest-first so `ml` is not swallowed by `l`, with a trailing `(?!\p{L})` so `l` does not match inside `lát`.
- A lot's conversion unit can CHANGE when a new lot is entered (Bột khoai môn moved ml → g). Existing recipe lines keep the old unit, and if the two units are in different families the item silently costs `undefined`, which makes the whole product read `Chưa đủ` for no stated reason — that is what hid 7 recipes. `productValidationErrors` now names the mismatch explicitly.
- Counting units are real conversion units, not a problem to work around: `vá`, `trái`, `miếng`, `muỗng`, `tờ`, `cái` are all in `COUNT_UNITS`, and Kho rows carry them (trân châu → `vá`, vải/nhãn hộp → `trái`). A recipe line may only use the NVL's own `Quy đổi sử dụng` unit, so check that before converting anything by hand.
- The SAPO price-book export MERGES the item-name cell across an item's price tiers, so every row after the first exports with a blank name. `parseCounterPriceRows` must carry the last name down and keep `Tên giá` as the tier — skipping nameless rows dropped every size-L price, landing the 66-row export as 48 rows and leaving đối soát valuing multi-size items at their smallest tier. `finance_counter_prices` is keyed by (name, size) since `20260913000100`.
- Invoice line items carry NO size, so đối soát cannot resolve which tier a basket item was. It resolves a bare name to the untiered row first, then S, M, L — the same number it produced before the fix, but now a deliberate choice rather than a side effect of which row survived.
- `Đồng bộ giá từ bảng giá SAPO` in Quản lý sản phẩm fills ONLY blanks — counter price when it is 0, a channel price when that channel has none — and reports, without overwriting, any SKU whose counter price differs from the book, plus any price-less SKU whose name matches nothing in the book. A SKU with a size variant must match a row of the same size; falling back across tiers is what made size L inherit size M's price.
- A SKU carries one price per sales channel: `selling_price` stays the counter price and `channel_prices` (jsonb, keys `grab`/`shopee`/`greensm`) holds the platform listings. Margin is always measured against money RECEIVED, never the listed price — net rates come from đối soát 2026-09-11 (`SALES_CHANNELS` in `lib/master-data.ts`): Quầy 100%, GrabFood 71,0% (24,538% + 4,5% thuế), ShopeeFood 67,0% (28,55% + 4,5%), GreenSM 95,5% (0% + 4,5%). Never blend the three into one rate. These rates cover commission and tax ONLY — seller-funded promotions and platform advertising are billed separately (Grab bills ads per DAY, not per order), so every margin the app shows is a ceiling and each channel note says `chưa gồm CTKM & MKT` on screen. The catalogue columns and the `price`/`margin` sort keys show the UNWEIGHTED mean across the channels that carry a price; per-channel volume is not stored, so this is explicitly not a revenue-weighted blended margin.
- The product detail overview is one row of Giá vốn / Giá bán trung bình / Biên lợi nhuận trung bình / Công thức, then one row per sales channel pairing its listed price with its own margin (Quầy, Grab, Shopee, GSM). Every channel margin is computed on money received, and a channel with no price shows `Chưa niêm yết` rather than being hidden, so a missing sàn price is visible at a glance.
- A SKU can be switched to `inactive` from the catalogue row. Inactive SKUs leave every normal view and live only behind the `⏸ Ngưng hoạt động` chip, stop raising NVL stock warnings, and are excluded from the Category chip counts. The state STICKS: `save_product_master` carries `p_status` and `reconcile_product_master_from_finance` keeps `inactive` instead of resetting to `active` (`20260915000100`) — without that, the next Production load reactivates everything, the same trap `name_overridden` hit. `__inactive` is a status filter, not a category, so the grouping code must skip its category comparison.
- A stock issue with exactly ONE replacement candidate is not a decision — `Thay thế` applies it directly instead of opening a one-row dropdown, and the waiting queue offers a single bulk action for all of them. Bulk replacement groups by product so each SKU produces one new recipe version, not one per swapped line.
- Product name and Category are editable — per SKU in the detail panel, or for a whole Category through the ✎ on its group header, which saves every SKU in the group. Editing either sets `name_overridden` / `category_overridden`, without which `reconcile_product_master_from_finance` rewrites both from the Finance snapshot on the next Production load and the rename silently disappears.
- A `CHECK` constraint cannot contain a subquery, so validating the shape of a jsonb map needs an `immutable` helper function that the `CHECK` calls (`product_channel_prices_valid`).
- Scrolling: a `position: sticky` bar must not carry `backdrop-filter`, which re-blurs everything moving underneath it every frame, and every horizontal strip (`.tabs`, `.categoryChips`, `.costTrendLabels`) sets `overscroll-behavior-x: contain` so a trackpad gesture drifting sideways cannot chain into the page scroll.
- The catalogue list opens with every Category group COLLAPSED, filters by Category through the same `inventory-category-tabs` chips Kho NVL uses, and sorts within each group by Mã SKU, Giá vốn, Giá bán or Biên gộp in either direction. A SKU whose theoretical COGS or selling price is missing has no comparable value, so it sorts to the bottom in both directions instead of being treated as zero. Picking a Category chip also expands that one group.
- `lib/uat-sync.ts` copies Production into the Local/UAT sandbox one way: it only READS Supabase (through `loadCogsCatalog`, which is upsert-free and reconciliation-free, plus `loadInventory`) and only WRITES browser storage, so there is no code path back to Production. The button is rendered only when `uatMode` is true. It must write the Finance product snapshot as well as the master state — the UAT loader rebuilds imported SKUs from that snapshot and keeps only `manual` rows out of storage, so writing the master state alone loses every imported SKU on the next reload. It signs in to Supabase for the read and signs out afterwards, because the UAT gate is a local password and the master tables are RLS'd to `authenticated`, where an unauthenticated read returns zero rows rather than an error.
...
| Recipes from CT 01.09.26 | imports `Menu/BẢNG TỔNG HỢP CT 01.09.26.xlsx`. Scope rule: a SKU whose recipe already computes a COGS is left untouched; only SKUs missing their constituent NVL get the workbook formula. Creates 3 placeholder NVL branded `CẦN THAY THẾ` (Sâm dứa, Muối hồng, Cốt chanh, stock 0), applies the workbook to 10 SKUs that had packaging but no ingredients (8 Sâm Dứa, Trà tắc chanh dây, Trà dâu), and adds 27 SKUs including the new `Soda` (12) and `Nước tươi` (4) categories, all at price 0 so the catalogue flags them. Verified on Production: 37/37 touched SKUs compute a COGS | `20260912000300` |
| Product active status | lets a SKU be deactivated and makes it stick: `save_product_master` gains `p_status`, and `reconcile_product_master_from_finance` preserves `inactive` instead of forcing `active` on every Production load | `20260915000100` |
| Counter price size tier | adds `finance_counter_prices.size` and re-keys the table on (name, size) so an item's price tiers stop colliding; pairs with the parser fix that carries SAPO's merged item name down across tier rows | `20260913000100` |
| Product channel prices | adds `product_master.channel_prices` (jsonb, `grab`/`shopee`/`greensm`), `name_overridden` and `category_overridden`; validates the price map through the immutable `product_channel_prices_valid` because a CHECK cannot hold a subquery; widens `save_product_master` to carry all three and to drop blanked-out platform prices instead of storing zeros; teaches `reconcile_product_master_from_finance` to leave an overridden name or category alone | `20260912000200` |
| Prepared-component output guard | repairs the `Công thức nền` SKUs that vanished from the picker. Sets the 7 Syrup Mứt active versions to `2000 ml` (1000 ml syrup + 1000 ml mứt per batch, confirmed by Long 2026-09-12), normalises 7 archived versions carrying an orphan `output_unit` back to `(null, null)`, rewrites `product_recipe_versions_output_check` so the half-filled pair evaluates FALSE instead of NULL, and makes `save_product_recipe_version` raise when quantity and unit are not both present or absent | `20260912000100` |
