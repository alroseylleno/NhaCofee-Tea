...
- The recipe cascade offers only active current-inventory master rows with sealed stock greater than zero: Category is sourced from those in-stock rows, then the ingredient/brand picker is constrained to the selected Category. Inactive or out-of-stock historical rows remain resolvable by saved recipes but are never selectable for new recipe items.
- Ingredient Master has no manual activation queue. Inventory ingredients become available to new recipe items as soon as they have appeared in Kho NVL and have stock greater than zero; a saved recipe that later reaches zero stock raises a replacement alert and cannot be saved as a new version until the ingredient is replaced or removed.
